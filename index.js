...
async function generateThreePublicKeys() {
  const collection = new Array(3).fill(0);

  const publicKeysCollection = collection.map(() => {
    const mnemonic = KeyGen.generateMnemonic(256);
    const masterKey = KeyGen.getMasterKeyFromMnemonic(mnemonic);
    const derivedPrivate = masterKey.derivePath("m/44'/0'/0'/0");
    const publicKey = KeyGen.getPublicKeyFromPrivateKey(derivedPrivate);

    return publicKey.toJSON().xpubkey;
  });
  await writeFileAsync(`${rootDir}/pub_keys.json`, JSON.stringify(publicKeysCollection), 'utf8');
}
